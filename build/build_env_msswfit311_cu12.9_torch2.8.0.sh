...
uv pip install ms-swift==3.11.1 pybind11 ninja wandb
...
# Ensure libnccl.so exists for linking (it might only be libnccl.so.2)
if [ ! -f "$NCCL_ROOT/lib/libnccl.so" ]; then
    ln -s "$NCCL_ROOT/lib/libnccl.so.2" "$NCCL_ROOT/lib/libnccl.so"
fi

# transformer_engine
# 若出现安装错误，可以参考该issue解决: https://github.com/modelscope/ms-swift/issues/3793
uv pip install --no-build-isolation --no-cache transformer_engine[pytorch]==2.10
# 或使用以下方式安装
...
# megatron-core
uv pip install git+https://github.com/NVIDIA/Megatron-LM.git@core_r0.15.0
